...
Future<void> _aesEncrypt(
    QuickJsRuntime2 engine, String message, String key, int callbackId) async {
  try {
...
Future<void> _aesDecrypt(
    QuickJsRuntime2 engine, String ciphertext, String key, int callbackId) async {
  try {
    final keyBytes = Uint8List.fromList(utf8.encode(key));
...
class AesCrypt {
  ...
  void _invMixColumns(List<int> state) {
    for (var i = 0; i < 4; i++) {
      final col = [state[i], state[i + 4], state[i + 8], state[i + 12]];
      state[i] = _gmul(col[0], 14) ^ _gmul(col[1], 11) ^ _gmul(col[2], 13) ^ _gmul(col[3], 9);
      state[i + 4] = _gmul(col[0], 9) ^ _gmul(col[1], 14) ^ _gmul(col[2], 11) ^ _gmul(col[3], 13);
      state[i + 8] = _gmul(col[0], 13) ^ _gmul(col[1], 9) ^ _gmul(col[2], 14) ^ _gmul(col[3], 11);
      state[i + 12] = _gmul(col[0], 11) ^ _gmul(col[1], 13) ^ _gmul(col[2], 9) ^ _gmul(col[3], 14);
    }
  }
