...
class WebViewScope<T> internal constructor(
    ...
    /**
     * Loads [html] as the page content, with [baseUrl] as its origin. Can only be called once
     * per scope.
     *
     * Chromium rejects `navigator.serviceWorker.getRegistrations()` on documents created via
     * [WebView.loadDataWithBaseURL] ("document is in an invalid state"). AllManga/mkissa SPA
     * code awaits that promise — the uncaught rejection aborts chapterPages capture and
     * surfaces as [WebViewTimeoutException]. Stub ServiceWorker before site scripts run.
     */
    fun loadData(baseUrl: String, html: String, mimeType: String = "text/html") {
        markLoaded()
        val patched = injectServiceWorkerStub(html)
        android.util.Log.i(
            "KeiyoushiWebView",
            "host loadData SW stub applied base=$baseUrl html=${patched.length}b",
        )
        webView.loadDataWithBaseURL(baseUrl, patched, mimeType, "UTF-8", null)
    }
...
suspend fun getLocalStorage(url: String, key: String): String? = runWebView(timeout = 10.seconds) {
    val quotedKey = org.json.JSONObject.quote(key)
    onPageFinished {
        evaluateJs("localStorage.getItem($quotedKey)") { value ->
            resolve(
                when {
...
/**
 * Inject a ServiceWorker shim as early as possible. Chromium's native
 * `navigator.serviceWorker.getRegistrations()` throws InvalidStateError on
 * loadDataWithBaseURL documents — mkissa waits on that and never posts chapterPages.
 *
 * Prefer mutating the existing ServiceWorkerContainer methods (defineProperty often
 * fails when the property is already non-configurable on WebView).
 */
internal fun injectServiceWorkerStub(html: String): String {
    val stub =
        """
        <script>
        (function(){
          function emptyReg(){ return { unregister: function(){ return Promise.resolve(true); } }; }
          function patch(sw){
            if (!sw) return;
            try { sw.getRegistrations = function(){ return Promise.resolve([]); }; } catch(e){}
            try { sw.getRegistration = function(){ return Promise.resolve(undefined); }; } catch(e){}
            try { sw.register = function(){ return Promise.reject(new Error('serviceWorker disabled')); }; } catch(e){}
            try {
              Object.defineProperty(sw, 'ready', {
                configurable: true,
                get: function(){ return Promise.resolve(emptyReg()); }
              });
            } catch(e){}
            try { sw.controller = null; } catch(e){}
          }
          try { patch(navigator.serviceWorker); } catch(e){}
          try {
            var fake = {
              getRegistrations: function(){ return Promise.resolve([]); },
              getRegistration: function(){ return Promise.resolve(undefined); },
              register: function(){ return Promise.reject(new Error('serviceWorker disabled')); },
              get ready(){ return Promise.resolve(emptyReg()); },
              controller: null,
            ...
            try { patch(navigator.serviceWorker); } catch(e2){}
          }
        })();
        </script>
        """.trimIndent()
    // Absolute first script — before doctype/html if needed — so it runs before SPA.
    val head = Regex("<head([^>]*)>", RegexOption.IGNORE_CASE).find(html)
    return if (head != null) {
        html.replaceRange(head.range.last + 1, head.range.last + 1, stub)
    } else {
        stub + html
    }
}
