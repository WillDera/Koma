...
- **Android Gradle:** 9.5.1 / AGP 8.11.1 / Kotlin 2.4.10 / `compileSdk` follows Flutter's default (currently 35/36). The AGP/Kotlin versions will be dropped by Flutter soon — bump when convenient.
...
- **Keystore:** `android/app/koma-debug.keystore` is tracked in the repository so all builds share the same signing certificate, preventing "App not installed" update failures. The `koma` signing config in `build.gradle.kts` uses credentials (`k0m@k0m@`) to sign debug, profile, and release builds consistently.
...
Do **not** build the app. The user will build and report errors. Your job is to make sure `flutter analyze` after your changes does not include any errors. Only rely on `flutter analyze` for validation — never run `flutter build`, `flutter run`, or `./gradlew` (the JDK/toolchain environment is not guaranteed to match).
